...
\begin{document} 
	...
	\begin{abstract}
		\begin{quote}
			...
			In this article, we present a generalized approach to extend the use of word2vec for non-traditional NLP (natural language processing). To exemplify the idea, we use the tvtropes data set (only troop names and movie names) to create a text corpus in order to provide contextual information to any data. Two different approaches have been made when creating the text corpus: the first called N-Grams Permutation Model and the second Database Like Reinforcement Model. We have compared both approaches with a model based on text written by humans and we have verified that in the best case the artificial model improves on the one created by humans by 45.2\2%.
		\end{quote}
	\end{abstract}
...
% * Example of synthetic movie generation.
	...
	\subsection{Approach 1. Construction of the artificial corpus: N-Grams Permutation Model}
...
\subsection{Approach 2. Construction of the artificial corpus: Database Like Reinforcement Model}

...
\begin{document} 
 ...
 % This is what you say above - JJ	
	...
	The next step consist of creating an artificial corpus through the permutation of set M values taken from N in N. M corresponds to film number of tropes  (MAX\_TROPES) and N with the size of the phrase or NGRAM-SIZE. table \ref{tab:corpusSize} shows different sizes of NGRAM\_SIZE and MAX\_TROPES and we decided to use the values of 15 and 9 since it offers us a corpus size that we consider may be suitable for training.
	...
	\begin{enumerate}
		\item Create corpus by relationships reinforcement
		\item Build word2vec model. After that we will have a numerical vector for each trope
		\item Check model accuracy using generated check list
		\item Visualize tropes vector space in order to detect clusters and possible data organization.
	\end{enumerate}

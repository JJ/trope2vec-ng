...
\begin{document} 
    ...
    Each of these contents use resources and techniques that are studied and used repeatedly and they are called tropes. % You need to define tropes, and add a reference. Tropes are much more (and less) than simply "resources and techniques".  
    Being able to characterize these tropes so that they can be used within an computer algorithm can be very interesting for those creators who perform this type of artistic work. %How would they use it? - JJ
    ...
    With this technique, the vector obtained for each word has the ability to save memory of its environment. % It does not "save" "memory" it's defined by the context it's in - JJ
    This vector is called embedding vector. % REFERENCE!!!! - JJ
    ...
    % If this is what you want to do, lay out an use case for this kind of things. - JJ
    
    But, it would be possible to apply word2vec to represent represent the relationships between tropes and films? % PLEASE CORRECT GRAMMAR.
    ...
    In order to solve the problem, we have downloaded films data from word2vec. %using... reference Rubén!!! - JJ
 ...
 % This is what you say above - JJ	
	...
	\begin{enumerate}
		\item Getting film data from tvtropes.org and their associated tropes % using... - JJ
		...
		\item Generate accuracy check list % What?  JJ
	...
	\begin{table}[t]
		...
		\end{tabular}
		\caption{Characteristics of different corpus sizes and word2vec models created by varying MAX\_TROPES and NGRAM\_SIZE (word2vec argument min-count = 5, MIN\_TROPES = 2)} % Why do you need to do this - JJ
	...
	The word2vec algorithm is applied to the generated corpus. For
        ...
        as main parameters. % All choices need to be justified - JJ
	...
	This second technique of creating the relationships between films and tropes, making the name of the trope unique for each film, will be used in the second method of corpus creation. Finally, we take a group of movies randomly and associate their top trope. Next, we take first pair (movie, top-trope) and the other selected pairs and we repeat this process with all the films and their associated top tropes. We will use the same accuracy checklist for the two proposed corpus creation methods.   
        ...
        % Don't include tables from other papers or those that are off
        % topic - JJ
	\begin{table*}[ht]
		\centering
		\begin{tabular}{|p{0.65\linewidth}|p{0.35\linewidth}|}
			...
			\hline
		\end{tabular}
		\caption{Word2vec phrases to check model accuracy} % Use your own. - JJ
		\label{tab:phrases-check-accuracy}
	\end{table*}

        % It's going to be almost impossible for you to find a
        % checklist that's equivalent to this one. Unless you take
        % "Hamlet" and eliminate "medieval" and add "space" and you
        % get "Star Wars". But that's a (very) long shot. - JJ
	...
	In order to check the quality of generated embedding vectors,
        ...
        trope and a second movie. % you say how you created it, you
                                % don't say what. I don't think you're
                                % understanding correctly how to
                                % create the vector that refers to the
                                % movie, which should be created by
                                % adding its vectors. Also, I don't
                                % see the whole "accuracy" thing or
                                % why you use subsubsections in a
                                % conference paper (with 6 pages to
                                % boot) - JJ

        % Wait, that's all? No table? No checklist? You go to another
        % subsubsection that has nothing to do? - JJ
	...
	We have independently visualized some of these groups in a
        ...
        the tvtropes.org website. % And what are the results? - JJ
	
	\subsection{Construction of artificial corpus: relationships
          reinforcement approach}
        % Wait, what? Totally confused by now - JJ
	...
	\end{verbatim}
         ...
         % I STOP HERE. Totally lost. Don't know what you're doing,
         % why you're doing it, how this has got anything to do with
         % the introduction... JJ
